...
async function calculate(value){
  ...
  try{
    ...
    if(state.empleado?.rol==='ADMIN'){
      const c=data.calculoSueldo;
      if(c?.disponible){
        sueldoHtml = `<br><strong>🧮 Cálculo administrativo de sueldo</strong><br><br>
          Kilómetros de la corrida: <strong>${Number(c.km).toLocaleString('es-MX',{maximumFractionDigits:2})} km</strong><br>
          PP × KM: <strong>${escapeHtml(data.factor.ppTexto)} × ${Number(c.km).toLocaleString('es-MX',{maximumFractionDigits:2})} = ${money(c.pagoPP)}</strong><br>
          PK × KM: <strong>${escapeHtml(data.factor.pkTexto)} × ${Number(c.km).toLocaleString('es-MX',{maximumFractionDigits:2})} = ${money(c.pagoPK)}</strong><br><br>
          <strong>Sueldo estimado antes de impuestos: ${money(c.sueldoAntesImpuestos)}</strong><br><br>
          <small>Estimación antes de impuestos y de los descuentos de cartera que pudiera tener el colaborador.</small>`;
      }else{
        sueldoHtml = `<br>⚠️ <strong>Cálculo administrativo:</strong> ${escapeHtml(c?.mensaje||'No fue posible obtener el kilometraje de esta corrida.')}`;
      }
    }
