...
function currentFilters(){
  return {
    manager: cleanText($("managerFilter")?.value),
    brand: cleanText($("brandFilter")?.value),
    type: cleanText($("typeFilter")?.value),
    search: cleanText($("searchFilter")?.value).toLowerCase()
  };
}
function rowMatchesFilters(r,{manager,brand,search}){
  if(manager && cleanText(r.manager)!==manager) return false;
  if(brand && cleanText(r.brand)!==brand) return false;
  if(search){
    const hay=[r.manager,r.brand,r.account,r.center,r.agrupador].map(cleanText).join(" ").toLowerCase();
...
function filteredModel(){
  ...
  const f=currentFilters(),m=state.model;
  const allowCost=!f.type||f.type==="Costos";
  const allowExpense=!f.type||f.type==="Gastos";
  const allowXpv=!f.type||f.type==="Productividad XPV";
  const costRows=allowCost?[...m.costOperating,...m.costMaintenance].filter(r=>rowMatchesFilters(r,f)):[];
  const expenseRows=allowExpense?m.expenses.filter(r=>rowMatchesFilters(r,f)):[];
  const xpvRows=allowXpv?m.xpv.filter(r=>rowMatchesFilters(r,f)):[];
  return {f,costRows,expenseRows,xpvRows,costSummary:aggregateByManager(costRows),expenseSummary:aggregateByManager(expenseRows)};
}
...
function buildDeviations(fm=filteredModel()) {
  ...
  const out=[];
  fm.costSummary.forEach(r=>{if(r.pct2526!=null&&r.pct2526>threshold)out.push({type:"Costo",manager:r.manager,y2025:r.y2025,y2026:r.y2026,pct:r.pct2526,impact:Math.abs(r.diff2526)});});
...
function renderDashboard(){
  ...
  const topExpense=t.fm.expenseSummary.slice().sort((a,b)=>Math.abs(b.y2026)-Math.abs(a.y2026))[0];
  const findings=[
...
function drawTrend(){
  ...
  const fm=filteredModel();
  const hasFilters=!!(fm.f.manager||fm.f.brand||fm.f.type||fm.f.search);

  // Con filtros, reconstruimos la tendencia mensual desde las filas JD para que la gráfica sí responda.
  if(state.inputMode==="JD"){
    const upto=state.periodIndex+1;
    const f=fm.f;
    const allowCost=!f.type||f.type==="Costos";
    const allowExpense=!f.type||f.type==="Gastos";
    const catalog=state.model.catalog;
    const monthlyFor=(source,allow)=>{
      if(!allow)return Array(upto).fill(0);
      const arr=Array(upto).fill(0);
      const rows=source||[];
      for(let i=9;i<rows.length;i++){
        const r=rows[i]||[];
        const account=outputText(r[1]),center=outputText(r[2]),brand=outputText(r[4]);
        let manager=cleanText(r[3]);
        if(!manager){
          const ck=cleanText(r[2]);
          manager=cleanText(catalog?.get?.(ck))||"SIN CLASIFICAR";
        }
        const probe={manager,account,center,brand,agrupador:outputText(r[0])};
        if(!rowMatchesFilters(probe,f))continue;
        for(let m=0;m<upto;m++)arr[m]+=Math.abs(parseNumber(r[6+m]));
      }
      return arr;
    };
    const cost=monthlyFor(state.sources[SHEETS.COST],allowCost);
    const exp=monthlyFor(state.sources[SHEETS.EXPENSE],allowExpense);
    const all=[...cost,...exp];const max=Math.max(...all,1);
    const series=[];if(allowCost)series.push([cost,"#2563eb"]);if(allowExpense)series.push([exp,"#7c3aed"]);
    ...
    if(!series.length||(!all.some(v=>v)&&hasFilters)){ctx.fillStyle="#94a3b8";ctx.font="14px system-ui";ctx.textAlign="center";ctx.fillText("Sin datos para los filtros seleccionados",w/2,h/2);}
  }else{
    const rows=fm.costSummary.slice().sort((a,b)=>Math.abs(b.y2026)-Math.abs(a.y2026)).slice(0,6);
    if(!rows.length){ctx.fillStyle="#94a3b8";ctx.font="14px system-ui";ctx.textAlign="center";ctx.fillText("Sin datos para los filtros seleccionados",w/2,h/2);return;}
...
function renderFilters(){
  ...
  const managerValue=$("managerFilter").value,brandValue=$("brandFilter").value;
  const managers=[...new Set([...state.model.costSummary.map(x=>x.manager),...state.model.expenseSummary.map(x=>x.manager),...state.model.xpv.map(x=>x.manager)].filter(Boolean))].sort();
  $("managerFilter").innerHTML='<option value="">Todas las gerencias</option>'+managers.map(x=>`<option value="${x}">${x}</option>`).join("");
  const brands=[...new Set([...state.model.costOperating,...state.model.costMaintenance,...state.model.expenses,...state.model.xpv].map(x=>x.brand).filter(Boolean))].sort();
  $("brandFilter").innerHTML='<option value="">Todas</option>'+brands.map(x=>`<option value="${x}">${x}</option>`).join("");
  if(managers.includes(managerValue))$("managerFilter").value=managerValue;
  if(brands.includes(brandValue))$("brandFilter").value=brandValue;
}
